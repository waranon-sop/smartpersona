...
export const LanguageSection = memo(({ languages = [], updateArrayItem, addArrayItem, removeArrayItem }) => (
      ...
      {languages.map((lang, idx) => (
        <div key={lang.id || idx} className="flex items-center gap-3">
          ...
          <select 
            ...
            className="w-full border border-gray-200 bg-white rounded-xl px-3.5 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400"
...
export const CertificationSection = memo(({ certifications = [], updateArrayItem, addArrayItem, removeArrayItem }) => (
  <Section icon={Award} color="#f59e0b" title="ใบรับรอง / Certifications" defaultOpen={false}>
    <div className="space-y-4">
      ...
      {certifications.map((cert, idx) => (
        <div key={cert.id || idx} className="border border-amber-100 rounded-xl p-4 bg-amber-50/40">
...
export const ProjectSection = memo(({ projects = [], updateArrayItem, addArrayItem, removeArrayItem }) => (
  <Section icon={FolderGit2} color="#ec4899" title="โปรเจกต์ / Projects" defaultOpen={false}>
    <div className="space-y-4">
      ...
      {projects.map((proj, idx) => (
        <div key={proj.id || idx} className="border border-pink-100 rounded-xl p-4 bg-pink-50/40">
          <div className="flex items-center justify-between mb-3">
